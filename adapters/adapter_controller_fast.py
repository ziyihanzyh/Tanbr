...
class Adapter(nn.Module):
    ...
    def forward(self, x, expert_index, prob_dist=None):
        ...
        if prob_dist is not None:
            if self.config.routing_estimator == 'ht_nucb_routing':
                prob_dist = prob_dist.unsqueeze(1).repeat(1, batch_size, 1)
...
class AdapterController(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.config.routing_estimator == 'ht_nucb_routing':
            self.multi_routers = HT_NeuralUCB(self.config.num_adapters, 200, len(self.config.tasks), beta=1, lamb=1,rho=0.8)
        else:
    ...
    def forward(self, tasks, orig_tasks, hash_lbls, inputs, given_hidden_states=None, weight = None):
        ...
        if routing_estimator is None:
            ...
            if routing_estimator == 'task_routing':
                ...
                if weight is None:
                    weight = np.ones(self.config.num_adapters)
                    weight = weight/sum(weight)

                #pdb.set_trace()
                if self.training:
                    adapter_probs = self.multi_routers.take_action(weight,evaluate=False)
                else:
                    adapter_probs = self.multi_routers.take_action(weight,evaluate=True)

                adapter_probs = torch.tensor(adapter_probs).unsqueeze(0).to(self.config.device)
        ...
        if routing_estimator == 'parameter_averaging_routing' or routing_estimator == 'ht_nucb_routing':
            #(M,B,S,C)
