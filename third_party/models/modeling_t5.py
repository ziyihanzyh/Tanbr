...
class T5LayerFF(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None,task=None, task_embedding=None, t5_block_adapters=None, hash_lbl=None, orig_task=None):
...
class T5LayerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.train_adapters and self.train_adapter_blocks:
            ...
            y = self.layer_hyper_net(y, [t5_block_adapter.self_attention for t5_block_adapter in t5_block_adapters])
        layer_output = hidden_states + self.dropout(y)
...
class T5Block(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.layer[-1](hidden_states, encoder_hidden_states=encoder_hidden_states, task=task,
                                       task_embedding=task_embedding,
                                       t5_block_adapters=t5_block_adapters, hash_lbl=hash_lbl, orig_task=orig_task)
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if self.train_adapters and (self.unique_hyper_net or self.efficient_unique_hyper_net):
                ...
                t5_block_adapters = []
                for t_embedding in task_embedding:
                    t5_block_adapters.append(self.adapter_layers_hyper_net(t_embedding, i))
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    def __init__(self, config, adapter_config=None):
        ...
        if config.train_adapters:
            ...
            # decoder_adapter_config.routing_estimator = None
            # decoder_adapter_config.num_adapters = 1
    ...
    def forward(
        ...
        if self.train_adapters and isinstance(self.adapter_config, MetaAdapterConfig):
            task_embeddings_list = [self.task_embedding_controller(t) for t in task]
        ...
        if encoder_outputs is None:
            ...
            encoder_outputs = self.encoder(
                ...
                task_embedding=task_embeddings_list if self.train_adapters \
                                                                       and isinstance(self.adapter_config,
                                                                                      MetaAdapterConfig) else None,
                hash_lbl = hash_lbl,
                orig_task = orig_task
        ...
        if not return_dict:
            ...
            if self.train_adapters:
                ...
                # There are multiple encoder and decoder passes, so we take the first 24 ones
                self.adapter_config.complete_analysis_list.extend(self.adapter_config.analysis_list[0:24] + self.decoder_adapter_config.analysis_list[0:24])
                self.adapter_config.analysis_list = []
                self.decoder_adapter_config.analysis_list = []
