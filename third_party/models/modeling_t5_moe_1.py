...
from hyperformer.adapters.adapter_utils import Router, Baseline
from hyperformer.adapters.algo.HT_NeuralUCB import *
...
class T5LayerFF_MOE(nn.Module):
    def __init__(self, config, adapter_config=None):
        super(T5LayerFF_MOE, self).__init__() 
        self.num_experts = adapter_config.num_adapters
        ...
        # 专家网络：每个专家是一个简单的全连接网络
        self.experts = nn.ModuleList([T5DenseGatedActDense(config,adapter_config) for _ in range(self.num_experts)])
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
        self.router = None
        if adapter_config.routing_estimator != 'ht_nucb_routing':
            self.router = Router(adapter_config)
        #    self.router = HT_NeuralUCB(self.num_experts, 2000, len(self.tasks), beta=1, lamb=1,rho=0.6)
        #else:
            
        
        if adapter_config.routing_estimator == 'reinf_bl_routing':
            self.multi_baselines = Baseline(adapter_config)
        if adapter_config.routing_estimator == "skill_routing":
            self.skill_weights = nn.Parameter(torch.empty((len(adapter_config.tasks), adapter_config.num_adapters)).uniform_(-1e-3, 1e-3))
        if adapter_config.routing_estimator == 'cluster_routing':
            self.clusters = torch.randn((self.num_experts, adapter_config.model_dim)).to(adapter_config.device) * self.config.cluster_initscale
            self.momentum = adapter_config.cluster_momentum

    def get_expert_index(self, z, batch_size, task):
        routing_estimator = self.adapter_config.routing_estimator
        load_loss = torch.tensor(0.0).to(self.adapter_config.device) 
        supervised_loss = torch.tensor(0.0).to(self.adapter_config.device) 
        expert_index = []
        adapter_probs = []
        if routing_estimator is None:
            assert self.num_experts == 1
            #(M,B)
            expert_index = [0]*batch_size
        else:
            if routing_estimator == 'task_routing':
                expert_index = [self.task_to_adapter[task_i] for task_i in task]
            elif routing_estimator == 'random_routing':
                expert_index = torch.randint(self.num_experts, (batch_size,))
            elif routing_estimator == 'adamix_routing':
                expert_index = (torch.randint(self.num_experts, (batch_size,)),
                torch.randint(self.num_experts, (batch_size,)))
            elif routing_estimator == 'dselectk_routing':
                z_sent = torch.mean(z, dim=1)
                if self.training:
                    dselectk_expert_weights, load_loss = self.router(z_sent)
                else:
                    dselectk_expert_weights = self.router(z_sent)
            else:
                z_sent = torch.mean(z, dim=1) 
                new_x, adapter_logits, adapter_probs = self.router(z_sent)
                if self.training:
                    if routing_estimator == 'gs_st_routing':
                        U = torch.rand(adapter_logits.shape).to(self.adapter_config.device)
                        y = adapter_logits + (-torch.log(-torch.log(U + 1e-20) + 1e-20))

                        y = F.softmax(y / self.adapter_config.adapter_temp, dim=-1)
                        ...
                        y = y_dist.sample()
                        hard_mask = F.one_hot(y, num_classes=self.num_experts).float()
                        baseline_vals = self.multi_baselines(new_x.detach())
                        self.adapter_config.adapter_probs_list.extend([t.squeeze(0) for t in torch.split(adapter_probs, split_size_or_sections=1,dim=0)])
                        self.adapter_config.baseline_vals_list.extend([t.squeeze(0) for t in torch.split(baseline_vals, split_size_or_sections=1,dim=0)])
                        self.adapter_config.adapter_samples_list.extend([t.squeeze(0) for t in torch.split(hard_mask, split_size_or_sections=1,dim=0)])
                        _, expert_index = hard_mask.max(dim=-1)
                    
                    elif routing_estimator == 'switch_routing_2':
                        probs, expert_index = adapter_probs.topk(k=2,dim=-1)
                        adapter_probs = probs
                    
                    else:
                        probs, expert_index = adapter_probs.max(dim=-1)
                    
                    if self.adapter_config.use_load_balancing:
                        for router_index in range(self.adapter_config.num_routers):
                            indices_load = expert_index[router_index]
                            probs_load = adapter_probs[router_index]
                            mask_load = torch.zeros_like(probs_load)
                            mask_load[torch.arange(batch_size).to(self.adapter_config.device), indices_load] = 1.0
                    ...
                    if self.adapter_config.supervised_loss_weight != 0:
                        task_labels = [-100 if task=='None' else self.task_to_adapter[task_i] for task_i in task]
                        task_labels = torch.tensor(task_labels).to(self.adapter_config.device)
                        for router_index in range(self.adapter_config.num_routers):
                            supervised_loss += self.supervised_loss_fn(adapter_logits[router_index], task_labels)
                else:
                    #(M,B)
                    if routing_estimator == 'switch_routing_2':
                        probs, expert_index = adapter_probs.topk(k=2,dim=-1)
                        adapter_probs = probs
                    else:
                        probs, expert_index = adapter_probs.max(dim=-1)

        return expert_index, adapter_probs

    def expert_merging(self,models,weights):
        assert len(models) == len(weights)
        aggregated_model = T5DenseGatedActDense(models[0].config, models[0].adapter_config)
        with torch.no_grad():
            for name, param in aggregated_model.named_parameters():
                layer_params = [model.state_dict()[name].to(self.adapter_config.device) for model in models]
            
                weighted_sum = sum(weight * param for weight, param in zip(weights, layer_params))
                averaged_param = weighted_sum / sum(weights)
            
                param.copy_(averaged_param)
    
        return aggregated_model.to(self.adapter_config.device)

    def forward(self, hidden_states, encoder_hidden_states=None,task=None, hash_lbl=None, orig_task=None, weight =None):
        ...
        if routing_estimator != 'ht_nucb_routing':

            expert_index, expert_probs  = self.get_expert_index(norm_x,batch_size,task)
        ...
        if routing_estimator=='parameter_averaging_routing':
            y_list = []
            times = []
            for i in range(norm_x.size(0)):
                expert = self.expert_merging(self.experts, expert_probs[i])
                expert_output = expert(norm_x[i], encoder_hidden_states = encoder_hidden_states[i] if encoder_hidden_states is not None else None,
                                                       task = task[i] if task is not None else None, hash_lbl= hash_lbl[i] if hash_lbl is not None else None,orig_task = orig_task[i] if orig_task is not None else None) 
                y_list.append(expert_output)
            y = torch.stack(y_list)



        elif routing_estimator == 'ht_nucb_routing':
            expert = self.expert_merging(self.experts, weight)
            y = expert(norm_x, encoder_hidden_states = encoder_hidden_states, task = task, hash_lbl= hash_lbl,orig_task = orig_task) 


        else:
            if routing_estimator == "switch_routing_2":
                expert_probs = F.softmax(expert_probs, dim=-1)
                outputs = []
                for i in range(norm_x.size(0)):
                    o_1 = self.experts[expert_index[i][0]](norm_x[i], encoder_hidden_states = encoder_hidden_states[i] if encoder_hidden_states is not None else None,
                                                       task = task[i] if task is not None else None, hash_lbl= hash_lbl[i] if hash_lbl is not None else None,orig_task = orig_task[i] if orig_task is not None else None) 
                    o_2 = self.experts[expert_index[i][1]](norm_x[i], encoder_hidden_states = encoder_hidden_states[i] if encoder_hidden_states is not None else None,
                                                       task = task[i] if task is not None else None, hash_lbl= hash_lbl[i] if hash_lbl is not None else None,orig_task = orig_task[i] if orig_task is not None else None) 
                    outputs.append(o_1*expert_probs[i][0] + o_2*expert_probs[i][1])          
                y = torch.stack(outputs)
            else:
                y = torch.stack([self.experts[expert_index[i]](norm_x[i], encoder_hidden_states = encoder_hidden_states[i] if encoder_hidden_states is not None else None,
                                                       task = task[i] if task is not None else None, hash_lbl= hash_lbl[i] if hash_lbl is not None else None,orig_task = orig_task[i] if orig_task is not None else None) 
                         for i in range(norm_x.size(0))])
...
class T5LayerFF(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None,task=None, hash_lbl=None, orig_task=None, weight = None):
        ...
        if self.train_adapters:
            if self.training:
                y, load_loss, supervised_loss = self.adapter_controller(task, orig_task, hash_lbl, y, encoder_hidden_states, weight)
                self.adapter_config.load_loss_accm += load_loss
                self.adapter_config.supervised_loss_accm += supervised_loss
            else:
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, adapter_config=None, adapter = False):
        super().__init__()
        self.adapter_config = adapter_config
        self.is_decoder = config.is_decoder
        ...
        if self.is_decoder:
            ...
            # self.layer.append(T5LayerCrossAttention(config, \
            #                                         has_relative_attention_bias=has_relative_attention_bias))
        self.layer.append(T5LayerFF_MOE(config, self.adapter_config))
        '''
        if adapter:
            self.layer.append(T5LayerFF(config, self.adapter_config))
        else:
            self.layer.append(T5LayerFF_1(config, self.adapter_config))
        '''
    ...
    def forward(
        ...
        hidden_states = self.layer[-1](hidden_states, encoder_hidden_states=encoder_hidden_states, task=task,hash_lbl=hash_lbl, orig_task=orig_task, weight = weight)
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, adapter_config=None):
        ...
        self.block = nn.ModuleList(
            [T5Block(config, has_relative_attention_bias=bool(i == 0), adapter_config=self.adapter_config, adapter = True if i == 0 else False)
            for i in range(config.num_layers)]
        )
        self.train_adapters = config.train_adapters
    ...
    def forward(
        ...
        if self.adapter_config.routing_estimator == "ht_nucb_routing" and weight == None:
            weight = self.expert_prob
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    def __init__(self, config, adapter_config=None):
        ...
        encoder_config = copy.deepcopy(config)  #config.num_layers = 12
        ...
        if self.adapter_config.routing_estimator == "ht_nucb_routing":
            self.router = HT_NeuralUCB(self.adapter_config.num_adapters, 100, len(self.adapter_config.tasks), beta=1, lamb=1,rho=0.7)
    ...
    def assign_router_indices(self):
        ...
        if self.train_adapters:
            for block in self.encoder.block:
                ...
                router_index +=1 
                '''
                if hasattr(block.layer[-1], 'adapter_controller'):
                    block.layer[-1].adapter_controller.router_index = router_index
                    router_index += 1
                    if self.adapter_config.routing_estimator == 'ht_nucb_routing':
                        self.routers.append(block.layer[-1].adapter_controller.multi_routers)
    ...
    def get_expert_prob(self, weight=None,evaluate=False):
        if weight is None:
            weight = np.ones(self.num_experts)
            weight = weight/sum(weight)
        adapter_probs = self.router.take_action(weight,evaluate=evaluate)
        adapter_probs = torch.tensor(adapter_probs).unsqueeze(0).to(self.adapter_config.device)
        return adapter_probs[0]
    
    def set_expert_prob(self, expert_prob):
        self.encoder.expert_prob = expert_prob
        self.decoder.expert_prob = expert_prob
           
    
    def router_update(self, average_loss_per_task):
        self.router.update(average_loss_per_task)
        #for r in self.routers:
        #    r.update(average_loss_per_task)
