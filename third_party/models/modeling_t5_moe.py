...
class T5LayerFF_MOE(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None,task=None, hash_lbl=None, orig_task=None, weight =None):
        ...
        task_indices = [self.task_to_adapter[task_i] for task_i in task]

        y = torch.stack([self.experts[task_indices[i]](norm_x[i], encoder_hidden_states = encoder_hidden_states[i] if encoder_hidden_states is not None else None,
